...
module.exports.getEmployeeByNum = function(num){
    return new Promise((resolve, reject) => {
        var foundEmployee = employees.filter(employees => employees.employeeNum == num);
        resolve(foundEmployee[0]);
        if(newEmployees.length == 0)
            reject("no results returned");
    });
}
    ...
    return new Promise((resolve, reject) => {
        for (let i = 0; i < employees.length; i++) {
            if (employeeData.employeeNum == employees[i].employeeNum) {
                // copy from employeeData to employees[i];
                Object.assign(employees[i], employeeData);
                //assign the true/false to manager
